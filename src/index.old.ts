...
// IPA consonant to Compatibility Jamo mapping
 ...
 *
 * @param ipa - IPA notation string (e.g., "/ˈhɛloʊ/", "[həˈloʊ]")
 * @returns Korean Hangul pronunciation string
 *
 * @example
 * ipaToHangul("/ˈhɛloʊ/") // Returns: "헬로"
 * ipaToHangul("/kæt/") // Returns: "캣"
 * ipaToHangul("/ˈɪntərnɛt/") // Returns: "인터넽"
 */
export function ipaToHangul(ipa: string): string {
  ...
  // Parse IPA and remove stress markers and delimiters
  let cleanedText = '';

  for (let idx = 0; idx < preprocessed.length; idx++) {
    const char = preprocessed[idx];

    // Skip stress markers and delimiters
